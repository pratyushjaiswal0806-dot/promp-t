...
_COMPACTION_TRIGGER_THRESHOLD = float(
    os.environ.get("PROMPTCOMPILER_SESSION_TRIGGER_THRESHOLD") or "0.7"
)
_COMPACTION_SUMMARY_RATIO = float(
    os.environ.get("PROMPTCOMPILER_SESSION_SUMMARY_RATIO") or "0.22"
)


class SQLiteStore:
    """Local SQLite store for traces, sessions, and compile cache.

    Uses instance-level connection caching with WAL mode and busy timeout.
    Supports context manager protocol for explicit connection lifecycle.
    """
    ...
    def close(self) -> None:
        """Close the instance connection if it exists."""
    ...
    def record_trace(self, trace: dict[str, Any]) -> None:
        """Persist a request trace record."""
        self._execute(
            """
    ...
    def metrics(self, filters: dict[str, str]) -> dict[str, Any]:
        """Return aggregate usage metrics with optional field filters."""
        clauses: list[str] = []
        values: list[Any] = []
    ...
    def append_session_turn(
        ...
        Saves or updates the session record, inserts the turn, and triggers
        adaptive compaction if the token budget threshold is exceeded.
    ...
    def _maybe_compact_session(
        ...
        if total_session_tokens < int(target_token_budget * _COMPACTION_TRIGGER_THRESHOLD):
            return {"triggered": False, "summary_segment_id": None}
        ...
        summary_tokens = max(8, min(80, int(removed_tokens * _COMPACTION_SUMMARY_RATIO)))
