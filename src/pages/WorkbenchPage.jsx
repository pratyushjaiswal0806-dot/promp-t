...
const controls = [
  { title: "Model picker", body: "Shows the target model because token accounting, routing, and context limits depend on where the optimized prompt will run." },
  { title: "Compression mode", body: "Lets users choose whether the compiler should stay lossless, balance savings with meaning, or make aggressive reductions." },
  { title: "Token budget", body: "Turns optimization into a measurable target instead of a vague shorter-is-better request." },
  { title: "Retrieval top-k", body: "Keeps only the most relevant RAG chunks when policy allows semantic or lexical pruning." },
  { title: "Output format", body: "Shapes the optimized prompt for plain text, JSON-like payloads, or downstream application contracts." },
  { title: "Dry run", body: "Explains potential savings and warnings without changing the final prompt handed to the user." },
];
  ...
  return (
    ...
    >
      <SectionBlock
        ...
        eyebrow="Controls"
        title="Controls exist to make optimization explicit"
        note="Each control maps to compiler behavior, not decoration. Users can predict why the output changed."
      >
        <FeatureGrid items={content.controls} fallback={controls} />
      </SectionBlock>
      <SectionBlock id="workbench-flow" eyebrow="Workflow" title="The daily compile loop">
        <StepGrid steps={content.workflow} fallback={workflow} />
      </SectionBlock>
      <SectionBlock id="workbench-panels" eyebrow="Panels" title="What each workspace region is responsible for">
